...
##### Part 2 #####
def FindIntegers(input):
    total = 0
    for row in input:
        sorted_row = sorted(row, reverse=True)
        for i in range(len(sorted_row)):
            divisable = CheckDivisable(sorted_row[i:len(sorted_row)])
            if divisable:
                total = total + int(divisable)
                break
    return total

def CheckDivisable(row):
    for check in row[1:len(row)]:
        if (row[0]/check).is_integer():
            return int(row[0]/check)

if __name__ == '__main__':
    spreadsheet = sanitizeInput(input)
    print("Day 2 Part 1: %i" % CheckSum(spreadsheet))
    print("Day 2 Part 2: %i" % FindItegers(spreadsheet))
